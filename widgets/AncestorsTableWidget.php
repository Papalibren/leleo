...
class AncestorsTableWidget extends Widget
{
    /** @var \app\models\Cat */
    public $model;
    ...
    public function run()
    {
        ...
        // Решётка ячеек: $cells[row][col] = ['cat'=>Cat,'rowspan'=>int,'hasFather'=>bool,'hasMother'=>bool]
        $cells = [];
...
HTML;
        ...
        for ($row = 1; $row <= $totalRows; $row++) {
            ...
            for ($col = 1; $col <= $this->maxGenerations; $col++) {
                ...
                if (isset($cells[$row][$col])) {
                    ...
                    $photoUrl = $cat->getFirstPhoto()
                        ? '/' . $cat->getFirstPhoto()->image_path
                        : '/img/default-cat.webp';

                    $nurseryLink = '';
                    if ($cat->breeder && $cat->breeder->nursery) {
                        $nursery = $cat->breeder->nursery;
                    ...
                    echo '<br><small class="text-muted">' . Html::encode($cat->pedigree_number) . '</small>';
                    echo $nurseryLink;
                    ...
                    if (empty($cell['hasFather'])) {
                        echo Html::a(
                            ...
                            "/user/cat/create-parent?type=father&child_id={$cat -> id}",
                            [
                    ...
                    if (empty($cell['hasMother'])) {
                        echo Html::a(
                            ...
                            "/user/cat/create-parent?type=mother&child_id={$cat -> id}",
                            [
     ...
     *
     * @param array $cells
     * @param array $node ['cat'=>Cat,'father'=>...,'mother'=>...]
