...
namespace app\controllers\mx\dog;
...
class CreateController extends Controller
{
    ...
    public function actionSearchFather()
    {
        ...
        $q = Yii::$app->request -> get()['Dog']['father_id'];

        $gender = ['gender' => 'кобель'];

        $view = 'father/search';


        if(is_numeric($q)){
            $dogs = Dog::find()->where(['id' => $q])->andWhere($gender)
            ->orderBy('name ASC')->all();
        }else{
            $dogs = Dog::find()->where(['like', 'name', "$q"])->andWhere($gender)
            ->orderBy('name ASC')->all();
        }

        return $this->renderPartial($view, compact('dogs'));
    ...
    public function actionSearchMother()
    {
        ...
        $q = Yii::$app->request -> get()['Dog']['mother_id'];
    ...
    public function actionSearchOwner()
    {
        ...
        $q = Yii::$app->request -> get()['Dog']['owner_id'];
    ...
    public function actionSearchBreeder()
    {
        ...
        $q = Yii::$app->request -> get()['Dog']['breeder_id'];
    ...
    public function actionSelectFather()
    {
        ...
        if(Yii::$app -> request ->get('father_id')){
            ...
            $Dog = Dog::findOne($id);
            $placeholder = $Dog -> name . " id: " . $Dog->id . " pedigree :" . $Dog->pedigree_number;
            $view = 'father/select';
