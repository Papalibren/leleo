...
<div class="container mt-4">
    <div class="card animal-card p-4">
        <?php if (!$model->owner): ?>
            <div class="row">
                <div class="col-12 text-end">
                    <a class="btn btn-primary btn-sm" href="/dogs/edit/?id=<?= $model->id ?>" title="Просмотр">
        ...
        <div class="row">
            <div class="col-12 col-lg-6">
                <p><strong>Порода:</strong> <?= Html::encode($model->breed) ?></p>
                ...
                <p><strong>Окрас:</strong> <?= $model->color ? Html::encode($model->color->name) : 'Не указан' ?></p>
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        <?php if (!$hasFather): ?>
                            <?= Html::a(
                                ...
                                "/user/dog/create-parent?type=father&child_id={$model->id}",
                                [
                        ...
                        <?php if (!$hasMother): ?>
                            <?= Html::a(
                                ...
                                "/user/dog/create-parent?type=mother&child_id={$model->id}",
                                [
                ...
                if ($model->is_for_sale) {
                    $announcementLinks[] = Html::a(
                        ...
                        Url::to(['/announcement/index', 'animal_type' => 'dogs', 'type' => 'sale', 'dog_id' => $model->id])
                    );
                }
                if ($model->is_for_mating) {
                    $announcementLinks[] = Html::a(
                        '<span class="badge bg-primary me-1">Для вязки</span>',
                        Url::to(['/announcement/index', 'animal_type' => 'dogs', 'type' => 'mating', 'dog_id' => $model->id])
                    );
                }
    ...
    <div class="tab-content mt-2" id="animalTabContent">
        <div class="tab-pane fade show active" id="pedigree" role="tabpanel">
            <?= \app\widgets\AncestorsTableWidgetDog::widget(['model' => $model]) ?>
        </div>
        <div class="tab-pane fade" id="siblings" role="tabpanel">
            <?= \app\widgets\SiblingsWidgetDog::widget(['model' => $model]) ?>
        </div>
        <div class="tab-pane fade" id="offspring" role="tabpanel">
            <?= \app\widgets\ChildrenWidgetDog::widget(['model' => $model]) ?>
        </div>
    </div>
</div>